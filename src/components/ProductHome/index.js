...
const ProductHome = ({ data }) => {
  ...
  return (
    <C.Container>
      <C.Thumb>
        <C.Img
          src={data.product.thumbnail.replace(/\w\.jpg/gi, "W.jpg")}
          width="400px"
        />
      </C.Thumb>

      <C.InfoProduct>
        <C.Title>{data.product.title}</C.Title>

        <C.Price>{formatCurrency(data.product.price, "BRL")}</C.Price>
        <p>
          em{" "}
          <C.Portion>
            10X {formatCurrency(data.product.price / 10, "BRL")} sem juros
          </C.Portion>
        </p>
      ...
      <C.Details>
        <C.Shipping
          className={`${data.product.shipping.free_shipping ? "free" : ""}`}
        >
        ...
        <C.Seller>
          <p>
            Vendido por
            <C.Link href={data.product.seller.permalink} target="_blank">
              {data.seller.nickname}
            </C.Link>
          </p>
          {data.product.seller.seller_reputation.metrics.sales.completed >
          50 ? (
            data.product.seller.seller_reputation.metrics.sales.completed >
            200 ? (
              data.product.seller.seller_reputation.metrics.sales.completed >
        ...
        <C.Stock>
          ...
          {data.product.sold_quantity === 1 ? (
            <span>Último disponível!</span>
          ) : data.product.sold_quantity === 0 ? (
        ...
        <C.MercadoPagoCont>
          {data.product.accepts_mercadopago ? (
            <>
              <C.Img
